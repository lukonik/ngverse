...
export class DatepickerInputComponent<T> implements ControlValueAccessor {
  ...
  registerOnChange(
    fn: (value: DpDateRange<T> | T | null) => void
  ): void {
    this._onChange = fn;
  }
  ...
  protected handleInputKeydown(event: KeyboardEvent): void {
    ...
    const keysToOpen = ['Enter', ' ', 'Spacebar', 'ArrowDown'];
  ...
  protected readonly placeholderText = computed(() =>
    this.placeholder() ??
    (this.selectionMode() === 'range' ? 'Select range' : 'Select date')
  );
  ...
  protected readonly displayValue = computed(() => {
    ...
    const start = range.start
      ...
      : 'Start';
    const end = range.end
      ? this._adapter.format(range.end, format)
      : 'End';
  ...
  protected readonly hasValue = computed(() => {
    ...
    const range = this._value() as DpDateRange<T> | null;
